...
public class Node {
	...
	/*
	 * nearestDepot is private because it allows us to forcibly update the other
	 * associated variables when we update the depot
	 */
	public void setNearestDepot(DepotNode near, double travelCost) {
		if (near == null) {
			...
			this.nearestDepotDistance = Math
					.sqrt((this.x - near.x) * (this.x - near.x) + (this.y - near.y) * (this.y - near.y));
